...
namespace Minvu.Snat.Domain
{
    public class MinvuValidators
    {
        ...
        public class ValidarCheckBox : ValidationAttribute, IClientValidatable
        {
            public override bool IsValid(object value)
            {
                bool valor = TypeHelper.ConvertToBoolean(value);
                //return base.IsValid(valor);
                return valor;
            }

            public IEnumerable<ModelClientValidationRule> GetClientValidationRules(ModelMetadata metadata, ControllerContext context)
            {
                string errorMessage = "Requerido";

                // The value we set here are needed by the jQuery adapter
                ModelClientValidationRule regla = new ModelClientValidationRule();
                regla.ErrorMessage = errorMessage;
                regla.ValidationType = "validarcheckbox";
                yield return regla;
            }
        }
        ...
        public class ValidarRut : ValidationAttribute, IClientValidatable
        {
            public override bool IsValid(object value)
            {
                ...
                if (RutHelper.ValidaRutValidator(crut, dv) || rut == null)
                    return true;
                else
                    return false;
            }
        ...
        public class ValidaRango : ValidationAttribute, IClientValidatable
        {
            ...
            public ValidaRango(string propiedadAcomparar, bool debeSerMayor, bool debeSerIgual, string errorMessage)
                : base(errorMessage)
            {
                this.nombrePropiedadAComparar = propiedadAcomparar;
                this.debeSerMayor = debeSerMayor;
                this.debeSerIgual = debeSerIgual;
            }
            ...
            public IEnumerable<ModelClientValidationRule> GetClientValidationRules(ModelMetadata metadata, ControllerContext context)
            {
                ...
                regla.ValidationParameters.Add("debeserigual", this.debeSerIgual);
                yield return regla;
            }
        }
    }
